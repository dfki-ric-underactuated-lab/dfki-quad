 ...
 */
template <typename T>
class PdChangableController : public LeafSystem<T>, public StateFeedbackControllerInterface<T> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(PdChangableController)
   ...
   * dimensions.
   */
  PdChangableController(int size);
   ...
   */
  PdChangableController(const MatrixX<double>& state_projection);
   ...
   */
  PdChangableController(const MatrixX<double>& state_projection, const MatrixX<double>& output_projection);

  /** Scalar-converting copy constructor.  See @ref system_scalar_conversion. */
  template <typename U>
  explicit PdChangableController(const PdChangableController<U>&);

  /**
   * Returns the input port for the estimated state.
   */
  const InputPort<T>& get_input_port_estimated_state() const final { return this->get_input_port(input_index_state_); }
   ...
   */
  const OutputPort<T>& get_output_port_control() const final { return this->get_output_port(output_index_control_); }
  const InputPort<T>& get_input_port_kp() const { return this->get_input_port(input_index_kp_); }
  const InputPort<T>& get_input_port_kd() const { return this->get_input_port(input_index_kd_); }

 private:
  template <typename>
  friend class PdChangableController;
  ...
  int input_index_kp_{-1};
  int input_index_kd_{-1};
};
