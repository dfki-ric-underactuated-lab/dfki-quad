...
def process_rosbags_in_directory(result_datas : list[dict], path : str, tag_parser_fun, process_rosbag_funs : list):
    ...
    for exp_folder in os.listdir(path):
        ...
        failed_file = None
        try:
            failed_file = open(os.path.join(path,exp_folder, "failed_attempts.txt"), "r")
        except:
        ...
        for exp_bag in os.listdir(os.path.join(path, exp_folder)):
            ...
            if os.path.isfile(os.path.join(path, exp_folder, exp_bag)):
                ...
                print(f"Mark this experiment as experiment failed: {exp_folder} -> {exp_bag} ")
                failed = True
            try:
                            ...
                            if connection.topic.__contains__(topic):
                                for topic_field in bag_time_data[topic].keys():
                                    if topic_field == "recv_timestamp_ns":
                                         bag_time_data[topic][topic_field].append(timestamp)
                                    else:
                                        bag_time_data[topic][topic_field].append(msg.__dict__[topic_field])
                                break
                    # first get the keys:                    
                    tag_data = tag_parser_fun(exp_folder, exp_bag, bag_time_data)
                    for result_data_out, result_fun in zip(result_datas, process_rosbag_funs):
...
def calculate_sfpw(solve_time : np.ndarray, cpu_measurements : np.ndarray):
    ...
    cpu_measurements_interp = np.interp(solve_time[0,:], cpu_measurements[0,:], cpu_measurements[1,:])
    # get theoretical solve frequency
    solve_freq = 1. / solve_time[1,:]
    # get sfpw
